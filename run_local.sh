...
startDockerNetwork() {
	if [ -z "$DOCKER_NETWORKED_CHECKED" ]; then
		...
		if ! docker network ls | grep -s "simplecardgamenet"; then
			verbosePrint "Starting docker network simplecardgamenet on 10.13.62.0/24"
			docker network create -d bridge --subnet 10.13.62.0/24 --gateway 10.13.62.1 "simplecardgamenet"
		else
			verbosePrint "Docker network simplecardgamenet already running"
		fi
	fi
}
...
Url: http://localhost:8080/scg

Details for components:
scg {Source:\"mvn\", Default-Type:\"local\", Version-Info: \"Tested with 3-jre-11\"}
  -t scg:local #build local and respect -j
  -t scg:docker:[TAG] #docker based build, default tag: latest, uses image https://hub.docker.com/_/maven
...
if [ -z "$1" ]; then
	...
	declare START_SCG=YES

	declare START_TOMCAT=YES

else
	ALL_COMPONENTS=(SCG TOMCAT)
	for comp in "$@"; do
...
if [ "$SKIP_HASH_CHECK" != "YES" ]; then
	if which md5 1>/dev/null; then
		declare SELF_HASH_MD5="7f41e32026bae116848436cd11ddc916"
...
verbosePrint "DEFAULT: TYPE_SOURCE_SCG=local"
TYPE_SOURCE_SCG=local
	...
	cat <<-EOF >Vagrantfile
		...
		Vagrant.configure("2") do |config|
		  ...
		  config.vm.provision "shell", inline: <<-SHELL
		    ...
		    echo "...then browse to http://localhost:8080/scg"
		  SHELL
		end
	EOF
	vagrant up
...
# MvnPlugin // scg
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
verbosePrint "MvnPlugin // scg"

if [ "$START_SCG" = "YES" ]; then
	...
	for typeSourceElement in "${array[@]}"; do
		...
		if [ "$comp" == "scg" ]; then
			TYPE_SOURCE_SCG=$type
			if [ "$TYPE_SOURCE_SCG" == "local" ]; then
				TYPE_SOURCE_SCG_PATH=$pathOrVersion
			else
				TYPE_SOURCE_SCG_VERSION=$pathOrVersion
			fi
		fi

	done

	if [ "$TYPE_SOURCE_SCG" == "docker" ]; then
		if [ -z "$TYPE_SOURCE_SCG_VERSION" ]; then
			TYPE_SOURCE_SCG_VERSION=latest
		fi

	fi

	verbosePrint "TYPE_SOURCE_SCG = $TYPE_SOURCE_SCG // TYPE_SOURCE_SCG_PATH = $TYPE_SOURCE_SCG_PATH // TYPE_SOURCE_SCG_VERSION = $TYPE_SOURCE_SCG_VERSION"
	...
	if [ "$TYPE_SOURCE_SCG" == "docker" ]; then
		...
		f_build() {
			verbosePrint "pwd=$(pwd)\ndocker run --name=scg --rm -v $(pwd):/usr/src/build -v "$(pwd)/localrun/.m2":/root/.m2 -w /usr/src/build $dockerImage:$TYPE_SOURCE_SCG_VERSION mvn $MVN_CLEAN $MVN_OPTS package"
...
if [ "$START_TOMCAT" = "YES" ]; then
	...
	if [ "$TYPE_SOURCE_TOMCAT" == "docker" ]; then
		...
		if [ ! -f ".tomcatPid" ]; then
			...
			verbosePrint "docker run --rm -d ${dockerAddLibRefs[@]} -p 8080:8080 -m 70M --net=simplecardgamenet --name=tomcat $ADD_HOST_INTERNAL   -v "$(pwd)/localrun/webapps":/usr/local/tomcat/webapps oglimmer/adoptopenjdk-tomcat:$TYPE_SOURCE_TOMCAT_VERSION"
			dockerContainerIDtomcat=$(docker run --rm -d ${dockerAddLibRefs[@]} -p 8080:8080 \
				-m 70M \
				--net=simplecardgamenet --name=tomcat $ADD_HOST_INTERNAL \
				-v \
				"$(pwd)/localrun/webapps":/usr/local/tomcat/webapps oglimmer/adoptopenjdk-tomcat:$TYPE_SOURCE_TOMCAT_VERSION)
